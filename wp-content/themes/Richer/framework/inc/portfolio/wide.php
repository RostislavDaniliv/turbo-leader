...
<div id="portfolio-content" class="portfolio-wide clearfix">
	...
	<?php if( get_post_meta( get_the_ID(), 'richer_embed', true ) == "" ){ ?>
			    ...
			    <?php } else {?>
				   ...
				   foreach ( $images as $att ) {
				    		// Show image
				    		echo "<div class='portfolio-image span3'><a href='". $att['full_url'] . "' rel='prettyPhoto[slides".$id."]' class='prettyPhoto' title='".$att['title']."'><img src='". $att['url'] . "' alt='".$att['alt']."'/><div class='overlay'></div></a></div>";
				    	}?>
				    </div>
			    <?php }
				} else {
