	...
	<div class="span8">
		<?php if( get_post_meta( get_the_ID(), 'richer_embed', true ) == "" ){ ?>
		    		...
		    		if(get_post_meta( get_the_ID( ), 'richer_gridlayout', true ) != "true") { ?>
						...
						<div id="portfolio-slider" class="flexslider">
						   ...
						   foreach ( $images as $att ) {
					    		// Show image
					    		echo "<li><a href='". $att['full_url'] . "' rel='prettyPhoto[slides".$id."]' class='prettyPhoto' title='".$att['title']."'><img src='". $att['url'] . "' alt='".$att['alt']."'/><div class='overlay'></div></a></li>";
					    	}?>
						   </ul>
				    	</div> 
				    	<?php if(get_post_meta( get_the_ID( ), 'richer_slider_thumb', true ) == "true") {?>
			    ...
			    <?php } else {?>
				   ...
				   foreach ( $images as $att ) {
				    		// Show image
				    		echo "<div class='portfolio-image span4'><a href='". $att['full_url'] . "' rel='prettyPhoto[slides".$id."]' class='prettyPhoto' title='".$att['title']."'><img src='". $att['url'] . "' alt='".$att['alt']."'/><div class='overlay'></div></a></div>";
				    	}?>
				    </div>
			    <?php }
				} else {
