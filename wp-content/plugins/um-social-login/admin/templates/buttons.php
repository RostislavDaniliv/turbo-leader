<div class="um-admin-metabox">
	...
	<p>
		<label class="um-admin-half"><?php _e('Assign Role','um-social-login'); ?></label>
		<span class="um-admin-half">
			
			<select name="_um_assigned_role" id="_um_assigned_role" class="umaf-selectjs" style="width: 300px">
			<?php $roles = $ultimatemember->query->get_roles(); ?>
			<?php $assigned_role = $ultimatemember->query->get_meta_value('_um_assigned_role'); ?>
			<?php foreach( $roles as $key => $value ): ?>
				<option data-default-role="<?php echo um_get_option('default_role'); ?>"  data-assigned-role="<?php echo $assigned_role; ?>" value="<?php echo $key;?>" <?php empty( $assigned_role  )? selected( um_get_option('default_role'), $key ) : selected( $key, $assigned_role ); ?>><?php echo $value;?></option>
			<?php endforeach; ?>
			</select>
